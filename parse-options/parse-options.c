...
static enum parse_opt_result get_value(const struct option* opt, const TCHAR* optarg)
{
...
static enum parse_opt_result parse_short_opt(int argc, const TCHAR* arg, const struct option* options)
{
	for (; options->type != OPTION_END; options++)
	{
		if (options->short_name == arg[0])
		{
			const TCHAR* optarg = (*arg + 1) ? arg + 2 : NULL;
...
static enum parse_opt_result parse_long_opt(int argc, const TCHAR* arg, const struct option* options)
{
	TCHAR* nameend;
...
int parse_options(int argc, const TCHAR** argv, const struct option* options)
{
	//if (argc == optind)
	...
	while (optind <= argc)
	{
		...
		/* return the index of the first non-option*/
		if (optind == argc)
		{
